# -*- encoding: utf-8 -*-
# net-http is at https://github.com/wycats/net-http
# 'This repo contains a number of experimental modifications to the version of Net::HTTP that ships with Ruby 1.9'
# It's still under development without releasing gems. Do not take the result so serious.
require 'net2/https'
...
class TestNet2HTTP < OdrkHTTPClientTestCase
  ...
  def test_ssl
    ...
    @client = Net2::HTTP.new('localhost', $ssl_port)
    @client.use_ssl = true
  ...
  def test_ssl_hostname
    ...
    @client = Net2::HTTP.new('127.0.0.1', $ssl_port)
    @client.use_ssl = true
    ca_file = File.expand_path('./fixture/ca_all.pem', File.dirname(__FILE__))
    @client.ca_file = ca_file
    assert_raise(OpenSSL::SSL::SSLError) do
  ...
  def test_post_multipart
    File.open(__FILE__) do |file|
      req = Net2::HTTP::Post.new(@url + 'servlet')
  ...
  def test_basic_auth
    req = Net2::HTTP::Get.new(@url + 'basic_auth')
  ...
  timeout(2) do
    ...
    c = Net2::HTTP.new(url.host, url.port)
    c.start
    begin
      5.times do
  ...
  def test_streaming_upload
    ...
    file.close
    file.open
    req = Net2::HTTP::Post.new(@url + 'chunked')
